...
/* in parallel, one temp file per mutation: 70-odd sequential suite runs took ~10 minutes */
...
const runSuite = file => new Promise(res => execFile('node', ['test.mjs', file],
  {encoding:'utf8', maxBuffer: 64 * 1024 * 1024}, (err, stdout, stderr) => res((stdout || '') + (err ? (stderr || '') : ''))));
...
async function worker(){
  while(next < M.length){
    ...
    else results[i] = (caught ? 'CAUGHT ' : 'MISSED ') + name;
  }
}
const t0 = Date.now();
