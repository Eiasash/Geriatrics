...
const M = [
  ...
  ['abbreviation in the pop-out stops closing the dialog',
   ...
   'abbreviation handler still falls back', undefined, {id:'m0012'}],
  ...
  ['pre-paint script accepts any text size',
   ...
   'pre-paint whitelist is in the source', undefined, {id:'m0015'}],
...
if(STATIC){
  ...
  /* survivesInjectedMarkup(label, term) is a thin wrapper around ok() (test.mjs ~3434, the
     Phase-4 search-highlighting refactor) — its own call sites pass a literal label straight
     through to ok(), but labelsOf's regex only sees a literal call's OWN name, not what it
     forwards to, so 'ok' alone misses these two labels entirely. Widened to the one known
     wrapper rather than to "any function", which would start matching unrelated calls that
     happen to take a string first argument. */
  const suite = labelsOf(fs.readFileSync('test.mjs', 'utf8'), '(?:ok|survivesInjectedMarkup)');
  /* a mutation whose runner is 'audit' is caught by a gate label in audit.mjs, not by a guard
     ...
     browser-check.mjs shares audit.mjs's exact check(label, value, isBad) signature, so the
     same labelsOf extraction applies to it unchanged. 'selfsrc' is the mutants.mjs self-check:
     its needle lives in production-path-guard.mjs, the file that actually implements the
     assertion this mutation exists to exercise — not a check()/ok() label file, so it stays on
     the raw decode() search rather than labelsOf. */
  const auditSrc = labelsOf(fs.readFileSync('audit.mjs', 'utf8'), 'check');
  const browserSrc = labelsOf(fs.readFileSync('browser-check.mjs', 'utf8'), 'check');
