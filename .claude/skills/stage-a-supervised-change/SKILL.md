...
# Supervised change loop â€” Stage A console
...
| **Cowork** (coordinator) | â€” | Relays, Gemini runs, verification, merges, live checks, project update doc, memory, scheduled checks. |
...
- **Report every change to shared infrastructure** when it lands: repo main, project docs, project memory, CI workflow, repo settings, this skill. Send one message to the project chat with what changed, where, and its revert handle:
  - repo â†’ merge commit (`git revert`)
  - project doc â†’ the previous doc path
  - memory â†’ the exact old text quoted
...
- **One bounded task at a time:** the fix, a guard, a red test (revert fix â†’ guard fails â†’ restore) and a `mutants.mjs` entry.
...
3. **Read `prNNN-review.txt`** and check it:
   - Exit 0 with a real review â†’ proceed.
   - "no output produced", a permission denial, an error, a refusal, an empty file, or a generic greeting â†’ it did not review. Fix the invocation and re-run; never treat it as "clean".
...
5. **Route findings:**
   - Mechanical verified defects â†’ CC as a new PR.
   - Anything touching content or scope â†’ the project chat first.
   - Tell Eias and the chat what was adopted or rejected, one line each with the reason.
6. **If `agy` stops working:**
   - Auth expired â†’ open it interactively in a visible window for Eias to sign in again.
   - Unavailable â†’ fall back to Eias pasting the prompt into his own Gemini and screenshotting the answer.
...
## Scheduling
- Use `send_later` / `update_trigger`, matched to the real work: CI about 5 min, CC edits 15â€“30 min, an `agy` review about 1 min.
...
## Doctrine â€” the guard ruleset (added 18 Sep 2026)
...
**Ranges are chosen per requirement.** An externally anchored requirement â€” an accessibility floor,
a sourced clinical figure â€” gets an absolute check. Relational ranges only for values that are
inherently about two things matching, and only once one side is externally anchored.

## The lanes â€” how to reach each one, and how to know it arrived
...
**NAMING â€” fixed by Eias on 19 Sep, and not to be drifted from, because ambiguous lane names made
two status reports unreadable.** **CHAT LANE** means the claude.ai PROJECT CHAT and nothing else:
it is the partner and the first stop, Eias's right hand, not a queue to be drained. The Claude Code
cloud session is the **CLOUD LANE**, or **the builder**. Never write "cloud lane", never call the
cloud session "the chat lane", and never say "chat lane" when you mean any other lane. Whatever a
lane is called, work goes to the chat lane FIRST and its answer is read in full before anything
moves onward â€” including a question the chat lane itself raised, which is the one that got skipped.

| Lane | Model / route | How to send | DELIVERY CHECK (target-side) | Known failure mode |
|---|---|---|---|---|
| **CHAT LANE** â€” the project chat, the partner, ALWAYS FIRST | Sonnet, claude.ai project chat | browser: focus the composer, `document.execCommand('insertText')`, click send | a new assistant turn appears after your message; it quotes or acts on your content | long replies stream for 2â€“4 min; reading too early returns only thinking summaries. The real failure is not technical: its own questions get answered to another lane instead of back to it |
| **CLOUD LANE** (the builder â€” Claude Code cloud) | Sonnet 5 / Medium, claude.ai/code | insertText, then **Ctrl+Enter** | **absence of "Queued. Claude will read this after the current turn"** AND no "Send now" control | a message typed while a task runs is QUEUED, not sent. Composer emptying and text-in-page are both TRUE while undelivered |
| **"Send now" / "Stop"** in that lane | â€” | ordinary clicks do nothing | after the action, re-check for the queued banner | needs a full dispatched pointer sequence: pointerdown, mousedown, pointerup, mouseup, click, with real client coords |
| **CC desktop** | `claude.exe -p --model opus --effort xhigh` | write prompt to a file, pipe it in via a `.cmd` wrapper, capture stdout to a file | the answer file is non-empty AND the log file records EXIT | PowerShell quoting mangles inline prompts â€” always use a `.cmd` wrapper and files, never inline quotes |
| **ChatGPT** | GPT-6 Astra, Extra High, project chat | attach the package as a FILE (`file_upload` to the hidden input), short instruction in the composer | **read the composer back: it must contain the payload.** A new response turn with content â€” it once returned an EMPTY answer after 6m28s of thinking | can refuse to verify counts against an unnamed commit â€” correct behaviour, pin the sha. **File delivery is fragile:** `file_upload` is refused for any path outside the extension's own read allowlist (a connected folder is NOT enough); a synthetic ctrl+v carries no clipboard payload; `navigator.clipboard.readText()` froze the renderer for 45s. All four paths failed on 19 Sep. **Never send the prompt without the payload â€” a review of an absent diff produces invented findings** |
| **Codex CLI** | `gpt-6-astra` | **NOT on PATH** â€” the binary is `C:\Users\eiasa\AppData\Local\OpenAI\Codex\bin\<hash>\codex.exe` (`where /r C:\Users\eiasa codex.exe` finds it). `.cmd` wrapper, same file-in/file-out pattern as CC desktop, run with the repo as cwd so no upload is needed | non-empty answer file | NOT independent of ChatGPT â€” same model family, so a second opinion rather than a second engine. It IS independent of Claude, which is the correlation that matters when the thing under review was designed by Claude lanes. **Repo hazard:** the repo-root `AGENTS.md` describes `shlav-a-mega.html`, a different app, and Codex reads it walking up from cwd â€” state the rules explicitly in the prompt and tell it to ignore `AGENTS.md` |
| **Gemini** | 3.1 Pro | **Antigravity IDE**, not the web app | the conversation appears in Antigravity's list and produces output | the WEB app's composer refuses programmatic text â€” its Angular model stays empty so Submit is a no-op. Chrome can only be granted READ access, so desktop-paste into it is blocked by design. Antigravity accepts a clipboard paste (`Set-Clipboard` then ctrl+v) |
| **Eias** | â€” | this chat | â€” | wants completions and decisions only, not narration |
...
Claude â€” one `engine` tag under R10, so their agreement is correlated, not corroborating. ChatGPT
...
## THE LANE RULE â€” binding, not advisory (18 Sep 2026)
...
2. **A decision that changes route goes back before it is acted on.** If the lane ruled on method
   A and method A fails, method B is a new decision â€” same outcome, different risk profile. Going
...
## O7â€“O12 â€” cross-lane rules (18â€“19 Sep 2026)

**Why these exist.** The coordinator acted on a stalled cloud lane only after Eias nudged. It had a
scheduled check on its OWN claims â€” main sha, PR draft state, ledger age â€” and nothing checking
whether the cloud lane was alive with an unanswered message in front of it. O1â€“O6 stopped the
...
posted that it was waiting on a check-in â€” the correct behaviour. So this is not two lanes
...
**O7 â€” Cross-lane liveness is not one party's job to remember.** Any lane depending on another
...
**O8 â€” Acting only after Eias intervenes is logged as this rule FAILING.** If the gap between "a
...
and acting within one tick of DISCOVERY â€” not one tick of being told.

**O9 â€” Self-reported blocking is standing across every lane.** A lane blocked on a check-in says so
...
*Worked example, and it is the reason O10 exists â€” keep it attached to O9 whenever this is
documented.* On 18 Sep the cloud lane did everything O9 asks: it finished item (2a), stopped, and
posted "not starting Â§3(3) without your check-in" as the last message in its thread. Visible
blocking, not silence. It was still missed for 53 minutes, because the reader on the other end
checked a STATUS FLAG â€” no Stop button, no queued message â€” instead of reading that message. So O9
...
**O10 â€” A status flag never substitutes for reading the peer's last content.** Busy/idle,
...
was â€” it is not a remedy for a failure the cloud lane demonstrated.*


**Convergence claims get MORE scrutiny, not less.** Two lanes reaching the same conclusion is this
project's preferred confidence signal â€” grounds to stop verifying and start fixing. So a claim that
...
*O10, generalised â€” attribution is checked against the source's own markers, not its content.* A
quote, a finding or a behaviour attributed to a party is verified against THAT PARTY'S ARTIFACT â€”
its header, title, thread name, commit author, run id â€” never against how well it completes the
...
same-bug narrative; the screenshot headers settle it in seconds â€” image 3 reads "SZMC geriatrics
...
**O11 â€” Coordinating a non-Claude lane gets the claims-log discipline pointed outward.** "Asked
Codex", "Gemini reviewed it", "ChatGPT confirmed" are each logged with the artifact supporting them
â€” a review comment with an id, a timestamped run, a quoted output â€” never "I sent it the prompt". A
report that an external lane reviewed, agreed or found nothing is UNSUPPORTED until the artifact is
named.

**O12 â€” Relay accuracy for external lanes is the coordinator's own.** A gap between what the
coordinator reports Codex CLI, Codex on GitHub, Gemini CLI, Gemini app or ChatGPT web said and what
the artifact shows is logged as the coordinator's failure, same class as the internal five â€” never
filed as the other model's mistake.

## Monitoring cadence â€” split by event class, not one flat poll
...
mergeable PR sitting behind a queued message â€” 25 minutes is far too slow for that class.
...
**A monitor's reference range is a guard's requirement â€” R7 applies to watchers too.** The external
watchdog's step 3 originally pinned main at a fixed sha. Every legitimate commit â€” including the one
that installed this very rule â€” invalidated it, so it would have fired, correctly by its own logic,
...
you push, re-read every monitor that references what you just changed** â€” the coordinator invalidating
...
- **Gemini via Antigravity is dropped** from the automatic loop at Eias's instruction. ChatGPT
  (GPT-6 Astra, Extra High) is the outside oracle. Codex CLI is not independent of it â€” same model
  family â€” so it is a second opinion, not a second engine.
- **Messages to a Claude Code cloud session are QUEUED while a task is running.** The composer
  ...
  convert its PRs to draft at GitHub (`gh pr ready <n> --undo`) â€” that holds whether or not any
...
- **"Send now" and "Stop" in that session do not respond to a normal automated click.** They need a
  full pointer sequence dispatched in the page â€” pointerdown, mousedown, pointerup, mouseup, click
  with real client coordinates â€” on the button element. Stop first, then Send now; verify by the
