...
|---|---|---|---|---|
| **Content lane** (project chat) | Sonnet, claude.ai project chat | browser: focus the composer, `document.execCommand('insertText')`, click send | a new assistant turn appears after your message; it quotes or acts on your content | long replies stream for 2–4 min; reading too early returns only thinking summaries |
| **Build lane** (Claude Code cloud) | Sonnet 5 / Medium, claude.ai/code | insertText, then **Ctrl+Enter** | **absence of "Queued. Claude will read this after the current turn"** AND no "Send now" control | a message typed while a task runs is QUEUED, not sent. Composer emptying and text-in-page are both TRUE while undelivered |
...
| **ChatGPT** | GPT-6 Astra, Extra High, project chat | attach the package as a FILE (`file_upload` to the hidden input), short instruction in the composer | a new response turn with content — it once returned an EMPTY answer after 6m28s of thinking | can refuse to verify counts against an unnamed commit — that is correct behaviour, pin the sha |
| **Codex CLI** | `gpt-6-astra`, `codex` on PATH | `.cmd` wrapper, same file-in/file-out pattern as CC desktop | non-empty answer file | NOT independent of ChatGPT — same model family. A second opinion, not a second engine |
...
**Independence, for the record:** the content lane, the build lane and the coordinator are all
...
1. **Nothing is issued before the project chat lane has attacked it.** Doctrine, rulesets,
   instruction sets to the build lane, ledger content, work orders. Agreeing the *substance* with
...
**Why these exist.** The coordinator acted on a stalled build lane only after Eias nudged. It had a
scheduled check on its OWN claims — main sha, PR draft state, ledger age — and nothing checking
whether the build lane was alive with an unanswered message in front of it. O1–O6 stopped the
...
message" self-diagnosis was the COORDINATOR's, not the build lane's. The build lane stopped and
posted that it was waiting on a check-in — the correct behaviour. So this is not two lanes
independently hitting one bug; it is the coordinator's fourth instance against a build lane that
...
build lane adopted this under pressure tonight; it now binds the coordinator, the content lane and
the build lane alike.

*Worked example, and it is the reason O10 exists — keep it attached to O9 whenever this is
documented.* On 18 Sep the build lane did everything O9 asks: it finished item (2a), stopped, and
...
in the reader, not the writer. Do not write this up as the build lane stalling. It did not stall.
...
was — it is not a remedy for a failure the build lane demonstrated.*
...
Worked example: a self-diagnosis was attributed to the build lane because it fitted a two-lanes-
...
verification" (the build lane). Checking the header is the check. It was available the whole time
