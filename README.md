...
Standalone, offline-capable PWA with 1,920 questions (1,580 real IMA exam + 340 AI-generated textbook questions), study notes, clinical calculators, FSRS-4.5 spaced repetition, and AI-powered explanations — all in one HTML file.

## Features

| Feature | Details |
|---|---|
| **Quiz Engine** | 1,920 questions with answer keys, topic tagging, and image support |
...
**Local:** Download and run `python -m http.server 3737`, open `http://localhost:3737/shlav-a-mega.html`
...
```
shlav-a-mega.html         # Complete app (v9.33, single-file PWA)
index.html                # GitHub Pages redirect
manifest.json             # PWA manifest
sw.js                     # Service worker (v9.33)
shared/fsrs.js            # FSRS-4.5 spaced repetition engine (shared with Pnimit Mega)
data/
  questions.json           # 1,920 exam + AI questions
...
tests/                     # Vitest test suite (296 tests, 7 files)
skill/                     # Claude Projects skill package
.github/workflows/         # CI: ci.yml (14 checks) + integrity-guard.yml (6 gates) + weekly-audit.yml
```
...
# Run tests
npm install && npm test   # 296 tests across 7 files

# Syntax check before push
node --check <(sed -n '/<script>/,/<\/script>/p' shlav-a-mega.html | sed '1d;$d')
```

No build step. Edit `shlav-a-mega.html`, push to main → GitHub Pages auto-deploys.

## Data Sources

- **Questions:** 1,580 MCQs from official IMA Shlav A exams (2021–2025) + 340 AI-generated textbook questions
