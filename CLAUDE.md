...
- **`scripts/chaos-doctor-bot-v4.mjs`** (~810 LOC) — Geri-native v4 judge bot. Selectors confirmed against the monolith (`button.qo`, `button.qo.ok`, `[aria-label*="check answer" i]`, `[aria-label*="next question" i]`/`[aria-label*="finish exam" i]`, `.heb`, `.explain-box` — aria-label selectors are case-insensitive substring matchers since #332, to survive a11y/i18n label drift à la #290); enters via the default practice surface so `data-state="correct"` is on the actual answer key, not the user's pick (the failure mode that produced 100% appIdx=null in v3). JSONL ledger writes to `chaos-reports/v4/medical_findings_ai_v4.jsonl`. Helper libs at `scripts/lib/extractJson.mjs` + `scripts/lib/optionResolver.mjs`. Test suite: `tests/chaosBotV4ExtractJson.test.js`, `tests/chaosBotV4OptionResolver.test.js`, `tests/chaosBotV4Persona.test.js`, `tests/chaosBotV4ProxyMode.test.js`, `tests/chaosBotV4ModalDismiss.test.js` — 25 tests across the v4 pins.

**Cross-repo quiz-markup divergence (selectors).** Geri + IM use `button.qo` / `button.qo.ok` (correct = `.ok`, wrong = `.no`). **FamilyMedicine diverged** (the v1.25.x BEM redesign — intentional; do NOT "fix" FM's markup to match): FM options are `.quiz-choice` with `data-state="correct" | "correct-unchosen" | "wrong" | "muted"`, and the explanation lives in `.quiz-feedback__body`. Consequence: the `button.qo` v4 bot above is **Geri-only** — point it at the FM URL and it finds 0 options and silently mis-reports. FM already has its own correctly-targeted bots (`scripts/chaos-doctor-bot-fm.mjs`, `scripts/chaos-live-bot-v2-fm.mjs` — both `DEFAULT_URL=…/FamilyMedicine/` — plus FM's repo `scripts/chaos-doctor-bot-v4.mjs` / `chaos-doctor-bot.mjs`) keyed to `.quiz-choice`/`data-state`. **Rule: match the bot to the target site's markup; never cross-point.** (Surfaced by the 2026-05-30 live smoke/real-world-sim audit.)

**Running:** `CHAOS_USE_PROXY=1 CHAOS_USERS=N CHAOS_DURATION_MS=ms CHAOS_MODEL=claude-sonnet-4-6 CHAOS_REPORT_DIR=path node scripts/chaos-doctor-bot-v4.mjs`. Default direct-Anthropic mode requires `CLAUDE_API_KEY`; proxy mode (v10.64.114+) routes through `https://toranot.netlify.app/api/claude` using `TORANOT_API_SECRET` env (defaults to the documented `RETIRED_PROXY_SECRET_2026_07_19` value). Proxy is the practical choice from CI / sandboxes / anywhere a personal Anthropic key isn't available.
...
All application logic lives in `shlav-a-mega.html` (~8,636 lines, 227 named functions) — no bundler, no framework, no build step. The file contains:
...
```
/
├── shlav-a-mega.html        # Main app (THE file — all HTML/CSS/JS, v10.64.195)
├── index.html               # GitHub Pages redirect → shlav-a-mega.html
├── sw.js                    # Service worker (offline caching + background sync)
├── manifest.json            # PWA manifest
│
├── data/                    # Lazy-loaded JSON data — single source of truth
│   ├── questions.json       # 4,647 MCQs (primary runtime source; explanations in data/explanations.json; 4,644/4,647 carry `ref` — 3 unsourced)
...
│
├── tests/                          # 108 vitest files, 1,596 tests + 7 skipped (see Testing section)
│
...
```json
{
  ...
  "e": "...",   // pre-generated AI explanation — MOVED to data/explanations.json (v10.64.93); not inline in questions.json
...
- Currently all three at `10.64.195` (sw.js cache key: `shlav-a-v10.64.195`)
...
### Testing
```bash
npm test             # Run all tests (vitest, across 113 files + 7 skipped)
npm run verify       # Pre-push gate (see below) — runs 7 checks in series
```

**across 113 files** — run `npm test` to see current count.
...
**Test file inventory (113 files — current as of v10.64.195; prior tally at v10.64.86 was 61 files / 1,270 tests):**
...
| Metric | Value |
|---|---|
| Main file | `shlav-a-mega.html` (~8,636 lines, ~580 KB) |
| Named functions | 227 (231 incl. shared/*.js, the integrity-guard counting basis) |
| Questions | 4,647 (IMA past exams + Hazzard/Harrison AI-generated + GRS8 imports; explanations in data/explanations.json; 4,644/4,647 carry `ref` — 3 unsourced) |
| Topics | 46 |
| Drugs | 113 |
| Flashcards | 159 |
| Study notes | 46 |
...
| Test suite | 1,602 tests across 108 files + 7 skipped (vitest) |
...
| App version | v10.64.195 |
| SW cache key | `shlav-a-v10.64.195` |
...
2. **Explanation quality checks** — Test that all entries in `data/explanations.json` are non-empty, >= 50 chars, contain no HTML injection, and are valid Hebrew/English text
